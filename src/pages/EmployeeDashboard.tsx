...
const EmployeeDashboard = () => {
    ...
    const checkEmployee = async () => {
      ...
      try {
        setIsCheckingEmployee(true);
          const response = await fetch('https://premier-paula-evolvoria-acf84abc.koyeb.app/core/api/employees/check/', {
          method: 'POST',
          headers: {
  ...
  useEffect(() => {
    const fetchAttendanceLogs = async () => {
      ...
      try {
        const response = await fetch('https://premier-paula-evolvoria-acf84abc.koyeb.app/core/api/employees/attendance-history/', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `JWT ${localStorage.getItem('auth_token')}`
  ...
  const confirmCapture = async () => {
    ...
    try {
      ...
      const uploadResponse = await fetch('https://premier-paula-evolvoria-acf84abc.koyeb.app/core/api/employees/upload/', {
        method: 'POST',
        body: formData,
        headers: {
          'Authorization': `JWT ${localStorage.getItem('auth_token')}`
  ...
  const analyzeCapturedImage = async (imageDataUrl: string) => {
    ...
    try {
      ...
      // Detect face in the captured image with lower threshold for better accuracy
      const detections = await faceapi
        .detectSingleFace(img, new faceapi.TinyFaceDetectorOptions({
          inputSize: 416,
          scoreThreshold: 0.3  // Lowered from 0.5 for better detection on cropped images
        }))
        .withFaceLandmarks();
      
      if (detections && detections.detection && detections.landmarks) {
        // Face detected - show mesh overlay
        console.log('✓ Face detected in captured image');
        console.log('Detection score:', detections.detection.score);
        ...
        // No face detected - provide more details
        console.log('✗ No face detected in captured image');
        console.log('Detections:', detections);
        if (detections?.detection) {
          console.log('Detection found but no landmarks:', detections.detection.score);
        }
        
        toast({
          title: "No Face Detected",
          description: "Ensure face is clear and visible. Returning to camera...",
          variant: "destructive"
        });
  ...
  return (
    <EmployeeLayout>
      <div className="max-w-4xl mx-auto space-y-6 px-4 pb-10 md:px-0">
        <div className="space-y-1">
          <h2 className="text-2xl md:text-3xl font-bold">Face Verification</h2>
