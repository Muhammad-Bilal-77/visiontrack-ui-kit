...
const Attendance = () => {
  ...
  const markAttendance = async () => {
    ...
    try {
      ...
      const uploadResponse = await fetch('https://premier-paula-evolvoria-acf84abc.koyeb.app/core/api/mark-attendance/', {
        method: 'POST',
        headers: {
  ...
  const getScaledFaceBox = () => {
    if (!faceBox || !videoRef.current) return null;
    ...
    // On small screens, enforce at least 40px left padding so box stays visible
    const minLeft = 40;
    if (scaled.x < minLeft) {
      const shift = minLeft - scaled.x;
      scaled.x = minLeft;
      // ensure we don't overflow the right edge when shifting
      if (scaled.x + scaled.width > clientWidth) {
        scaled.width = Math.max(0, clientWidth - scaled.x);
      }
      // optional: reduce width if still oversized
      if (scaled.width < 40) {
        scaled.width = 40;
      }
    }
  ...
  const scaledFaceBox = getScaledFaceBox();
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 p-4 md:p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-white">Mark Attendance</h1>
          ...
          <CardContent className="space-y-4">
            <div className="relative bg-gradient-to-br from-gray-900 to-gray-800 rounded-lg overflow-hidden flex items-center justify-center">
              <video
                ...
                playsInline
                muted
                className="rounded-lg w-full max-w-full md:max-w-[640px] h-auto"
              />
              ...
              <div className="absolute inset-0 pointer-events-none">
                ...
                {faceDetected && scaledFaceBox && (
                  <div 
                    className="absolute border-2 border-green-500 rounded-lg transition-all duration-300"
                    style={{ 
                      left: `${scaledFaceBox.x}px`,
                      top: `${scaledFaceBox.y}px`,
                      width: `${scaledFaceBox.width}px`, 
                      height: `${scaledFaceBox.height}px`,
                      boxShadow: '0 0 22px rgba(34, 197, 94, 0.5), inset 0 0 18px rgba(34, 197, 94, 0.15)',
                      animation: 'pulse 2s ease-in-out infinite'
                    }}
                  >
                    <div className="absolute -top-1 -left-1 w-3 h-3 md:w-4 md:h-4 border-t-4 border-l-4 border-green-400 rounded-tl" />
                    <div className="absolute -top-1 -right-1 w-3 h-3 md:w-4 md:h-4 border-t-4 border-r-4 border-green-400 rounded-tr" />
                    <div className="absolute -bottom-1 -left-1 w-3 h-3 md:w-4 md:h-4 border-b-4 border-l-4 border-green-400 rounded-bl" />
                    <div className="absolute -bottom-1 -right-1 w-3 h-3 md:w-4 md:h-4 border-b-4 border-r-4 border-green-400 rounded-br" />
                    
                    <div className="absolute -top-6 left-2 bg-green-500 text-white text-[10px] md:text-xs px-2 py-0.5 rounded shadow-sm">
                      FACE DETECTED
                    </div>
                  </div>
                )}
