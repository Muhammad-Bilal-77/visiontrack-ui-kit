    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold">Weekly Reports</h2>
            <p className="text-muted-foreground mt-1">View weekly attendance reports</p>
          </div>
          <div className="flex flex-wrap gap-2 sm:gap-3">
            <Button variant="outline" onClick={(e) => { e.preventDefault(); loadDailyReport(); }} disabled={loading} className="w-full sm:w-auto">Reload</Button>
            <Button variant="outline" onClick={() => downloadCSV(dailyRows, 'weekly_report')} disabled={dailyRows.length === 0} className="w-full sm:w-auto">
              <Download className="w-4 h-4" />
              Download CSV
            </Button>
            <Button variant="outline" onClick={() => downloadXLS(dailyRows, 'weekly_report')} disabled={dailyRows.length === 0} className="w-full sm:w-auto">
              <Download className="w-4 h-4" />
              Download XLS
            </Button>
          </div>
        ...
        <Card className="p-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" onClick={() => downloadCSV(dayEmployees, `day_report_${selectedDate}`)} disabled={dayEmployees.length === 0} className="w-full sm:w-auto">
                <Download className="w-4 h-4 mr-2" />
                CSV
              </Button>
              <Button variant="outline" onClick={() => downloadXLS(dayEmployees, `day_report_${selectedDate}`)} disabled={dayEmployees.length === 0} className="w-full sm:w-auto">
                <Download className="w-4 h-4 mr-2" />
                XLS
              </Button>
            </div>
