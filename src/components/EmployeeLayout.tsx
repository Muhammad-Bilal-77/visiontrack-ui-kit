...
const EmployeeLayout = ({ children }: EmployeeLayoutProps) => {
  ...
  useEffect(() => {
    ...
    const checkUserRole = async () => {
      try {
        const response = await fetch('https://premier-paula-evolvoria-acf84abc.koyeb.app/core/api/user-role/', {
          method: 'GET',
          headers: {
        ...
        if (result.success && result.data.role === 'employee') {
          ...
          try {
            const checkResponse = await fetch('https://premier-paula-evolvoria-acf84abc.koyeb.app/core/api/employees/check/', {
              method: 'POST',
              headers: {
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      <aside
        className={cn(
          "fixed left-0 top-0 h-screen w-64 bg-card border-r border-border z-40 transition-transform duration-200",
          isSidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
        )}
      >
        <div className="p-6 flex items-center justify-between md:justify-start md:gap-2">
        ...
        <header className="sticky top-0 z-20 bg-background/80 backdrop-blur-md border-b border-border">
          <div className="px-4 py-3 md:px-8 md:py-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl md:text-2xl font-bold">Employee Portal</h1>
            </div>
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="hidden sm:block">
                  <p className="text-sm font-medium truncate max-w-[140px] md:max-w-none">{employeeName}</p>
                  <p className="text-xs text-muted-foreground truncate max-w-[140px] md:max-w-none">{employeeEmail}</p>
                </div>
              </div>
            </div>
          </div>
